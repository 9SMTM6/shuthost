...
> **Note**: `frontend/index.html` is a dev-only file. The production HTML is generated by the Rust build script from `frontend/assets/page.template.html` and is never served directly.

### Linting and formatting

The frontend uses [Biome](https://biomejs.dev/) for linting and formatting:

| Task | Justfile shortcut | `pnpm` (from `frontend/`) |
|---|---|---|
| Type-check | `just tsc` | `pnpm run typecheck` |
| Lint (includes type-check) | `just frontend_lint` | `pnpm run lint` |
| Lint + auto-fix | `just frontend_lint_fix` | `pnpm run lint:fix` |
| Format | `just frontend_fmt` | `pnpm run fmt` |

Please run at least `just tsc` and `just frontend_lint` before submitting a pull request. The CI pipeline enforces both.
