...
4. **Configure the Script**:
   - Paste the following script into the script editor. Replace `<secret>` with the shared secret you noted in step 2, and adjust the coordinator URL and port as needed:
     ...
     curl -fsSL https://shuthost.example.com/download/host_agent_installer.sh | sh -s https://shuthost.example.com --os=linux-musl --port=5757 --shared-secret=<secret> --shutdown-command="echo -n mem > /sys/power/state" --init-system=serviceless
