...
pub async fn send_host_online_notifications(pool: &db::DbPool, host_name: &str) -> eyre::Result<()> {
    ...
    for subscription in subscriptions {
        ...
        let vapid_key = web_push::VapidSignatureBuilder::from_pem(vapid_keys.private_key.as_bytes(), &subscription_info)?;
        ...
        match response {
            Ok(resp) if resp.status().is_success() => {
                tracing::info!("Push notification sent successfully to {} for host {}", subscription.endpoint, host_name);
            }
            Ok(resp) => {
                tracing::warn!("Push notification failed for {}: status {}", subscription.endpoint, resp.status());
            }
            Err(e) => {
