...
use crate::{
    db, http::{AppState, m2m::{broadcast_lease_update, handle_host_state}}
};
...
async fn subscribe_push(
    ...
    if let Some(ref pool) = state.db_pool {
        ...
        match db::store_push_subscription(pool, &push_subscription).await {
            Ok(()) => {
                tracing::info!("Stored push subscription for endpoint: {}", subscription.endpoint);
...
async fn unsubscribe_push(
    ...
    if let Some(ref pool) = state.db_pool {
        match db::remove_push_subscription(pool, &subscription.endpoint).await {
            Ok(()) => {
                tracing::info!("Removed push subscription for endpoint: {}", subscription.endpoint);
                StatusCode::OK.into_response()
            }
            Err(e) => {
