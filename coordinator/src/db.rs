...
pub(crate) const KV_VAPID_PRIVATE_KEY: &str = "vapid_private_key";
pub(crate) const KV_VAPID_PUBLIC_KEY: &str = "vapid_public_key";
...
pub(crate) async fn get_or_generate_vapid_keys(pool: &DbPool) -> eyre::Result<VapidKeys> {
...
///
/// Returns an error if the database operation fails.
pub(crate) async fn store_push_subscription(
    pool: &DbPool,
...
///
/// Returns an error if the database query fails.
pub(crate) async fn get_push_subscriptions(pool: &DbPool) -> eyre::Result<Vec<PushSubscription>> {
