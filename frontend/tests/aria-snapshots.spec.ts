...
import {
    expand_and_sanitize_host_install,
    getBaseUrl,
    sanitizeEnvironmentDependents,
    sanitizeVersion,
} from './test-utils';
    ...
    test.describe(`${name} config`, () => {
        ...
        test(`ARIA snapshot for hosts tab (${name})`, async ({ page }) => {
            ...
            await expect(page.locator('#main-content')).toMatchAriaSnapshot({
                name: `at_hosts-cfg_${name}.aria.yml`,
            });
        });
        ...
        test(`ARIA snapshot for clients tab (${name})`, async ({ page }) => {
            ...
            await sanitizeVersion(page);
            await expect(page.locator('#main-content')).toMatchAriaSnapshot({
                name: `at_clients-cfg_${name}.aria.yml`,
            });
        });
    });
}
...
test.describe('expanded install panels', () => {
    ...
    test('ARIA snapshot with Install Host Agent expanded (nada)', async ({
        ...
        await sanitizeVersion(page);
        await expect(page.locator('#main-content')).toMatchAriaSnapshot({
            name: `cfg_${key}-at_hosts-expanded_install.aria.yml`,
        });
    });
    test('ARIA snapshot with Install Client expanded (nada)', async ({
        ...
        await sanitizeVersion(page);
        await expect(page.locator('#main-content')).toMatchAriaSnapshot({
            name: `cfg_${key}-at_clients-expanded_install.aria.yml`,
        });
    });
});
...
test.describe('no-auth landing page', () => {
    ...
    test('ARIA snapshot of root page (no-auth)', async ({ page }) => {
        ...
        await expect(page.locator('#main-content')).toMatchAriaSnapshot({
            name: `cfg_no-auth-root.aria.yml`,
        });
    });
    test('ARIA snapshot with security config expanded (no-auth)', async ({
        page,
    }) => {
        await page.goto(`${base}/`);
        await sanitizeVersion(page);
        ...
        await page.waitForSelector('#security-config-content', {
            state: 'visible',
        });
        await sanitizeEnvironmentDependents(page);
        await expect(page.locator('#main-content')).toMatchAriaSnapshot({
            name: 'cfg_no-auth-root-expanded-security.aria.yml',
        });
    });
});
...
test.describe('auth-outdated-exceptions landing page', () => {
    ...
    test('ARIA snapshot of security config (auth-outdated-exceptions)', async ({
        page,
    }) => {
        await page.goto(`${base}/`);
        await page.waitForSelector('#main-content', { state: 'attached' });
        await sanitizeVersion(page);
        await expect(page.locator('#main-content')).toMatchAriaSnapshot({
            name: `cfg_auth-outdated-exceptions-root.aria.yml`,
        });
        await page.click('#security-config-header');
        await page.waitForSelector('#security-config-content', {
            state: 'visible',
        ...
        await sanitizeEnvironmentDependents(page);
        await expect(
            page.locator('#security-config-content'),
        ).toMatchAriaSnapshot({
            name: 'cfg_auth-outdated-exceptions-expanded-security.aria.yml',
        });
    });
});
