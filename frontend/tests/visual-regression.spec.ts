...
test.describe('main page(s)', () => {
    ...
    test('expanded host install', async ({ page }) => {
        ...
        await page.waitForLoadState('networkidle');
        await expect(
            page.locator('section[aria-labelledby="host-install-title"]'),
        ).toHaveScreenshot(`at_hosts_expanded_install.png`);
    });
    ...
    test('platform support', async ({ page }) => {
        ...
        await expect(
            page.locator('section[aria-labelledby="platform-support-title"]'),
        ).toHaveScreenshot(`platform_support.png`);
    });
    ...
    test('javascript error display', async ({ page }) => {
        ...
        await expect(page.locator('#js-error')).toHaveScreenshot(
            'js_error_display.png',
        );
    });
    ...
    test('GPL2 license display', async ({ page }) => {
        await page.goto(`${base}/about`);
        await page.waitForLoadState('networkidle');
        const licenseSection = page.locator('#license-GPL-2\\.0-only').locator('..');
        await expect(licenseSection).toBeVisible();
        await expect(licenseSection).toHaveScreenshot(`gpl2_license_display.png`);
    });
});
...
test.describe('token login', () => {
    ...
    test('login page - session expired', async ({ page }) => {
        ...
        await sanitizeVersion(page);
        await expect(page.locator('#main-content')).toHaveScreenshot(
            `login_token_session_expired.png`,
        );
    });
    ...
    test('noscript warning when JS disabled', async ({ browser }) => {
        ...
        await expect(page.locator('#noscript-warning')).toHaveScreenshot(
            'noscript_warning.png',
        );
        await context.close();
    });
});
...
test.describe('OIDC login', () => {
    ...
    test('login page - session expired', async ({ page }) => {
        ...
        await sanitizeVersion(page);
        await expect(page.locator('#main-content')).toHaveScreenshot(
            `login_oidc_session_expired.png`,
        );
    });
});
